...
namespace toy::editor
{
    ...
    ContentBrowserDock::ContentBrowserDock(std::string &&dock_name)
    : m_dock_name{ std::move(dock_name) }, m_browser_cache_path{ s_root_path }
    {
        // TODO: handle asset manager
    ...
    void ContentBrowserDock::on_render()
    {
        ...
        {
            m_browser_cache_path = s_root_path.root_path();
        }

        const auto  browser_temp_path = m_browser_cache_path;
        ...
        ImGui::SliderFloat("##scale", &m_icon_scale, 0.5f, 1.0f);
        const float size = ImGui::GetFrameHeight() * 5.0f * m_icon_scale;
        if (ImGui::IsItemHovered())
        {
            ImGui::BeginTooltip();
    ...
    void ContentBrowserDock::set_browser_cache_path(const std::filesystem::path &path)
    {
        if (m_browser_cache_path != path)
        {
            m_browser_cache_path = path;
        }
    }
}