...
cbuffer SpecularMapFilterSettings : register(b0)
{
	...
	float  gRoughness;
	float3 gRoughnessPadding;
};

// // Input shader resource view from texture cube
...
// // Output unordered access view from environment map
...
void main(uint3 ThreadID : SV_DispatchThreadID)
{
	...
	gInputCubeMap.GetDimensions(0, inputWidth, inputHeight, inputLevels);
	...
	for(uint i = 0; i < NumSamples; ++i) 
	{
		...
		float3 Lh = tangentToWorld(sampleGGX(u.x, u.y, gRoughness), N, S, T);
		...
		if(cosLi > 0.0) {
			...
			float pdf = ndfGGX(cosLh, gRoughness) * 0.25;
			...
			color  += gInputCubeMap.SampleLevel(gSamAnisotropicWrap, Li, mipLevel).rgb * cosLi;
			weight += cosLi;
		}
	}
	color /= weight;

	gOutputCubeMap[ThreadID] = float4(color, 1.0);
}
