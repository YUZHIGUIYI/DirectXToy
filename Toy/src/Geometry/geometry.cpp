...
namespace toy::geometry
{
    ...
    GeometryData create_sphere(uint32_t entity_id, float radius, uint32_t levels, uint32_t slices)
    {
        ...
        float per_theta = 2 * XM_PI / float(slices);
        float x, y, z;
        ...
        for (uint32_t i = 1; i < levels; ++i)
        {
            ...
            for (uint32_t j = 0; j <= slices; ++j)
            {
                ...
                geoData.tangents[vIndex] = XMFLOAT4(-std::sin(theta), 0.0f, std::cos(theta), 1.0f);
                geoData.texcoords[vIndex++] = XMFLOAT2(theta / 2.0f / XM_PI, phi / XM_PI);
            }
        }
    ...
    GeometryData create_cylinder(uint32_t entity_id, float radius, float height, uint32_t slices, uint32_t stacks, float texU, float texV)
    {
        ...
        float theta = 0.0f;
        float per_theta = 2.0f * XM_PI / float(slices);
        ...
        //
        {
            ...
            {
                float y = -h2 + float(i) * stackHeight;
                // 当前层顶点
                for (size_t j = 0; j <= slices; ++j)
                {
                    theta = float(j) * per_theta;
                    float u = theta / 2 / XM_PI;
                    float v = 1.0f - float(i) / float(stacks);

                    geoData.vertices[vIndex] = XMFLOAT3(radius * std::cos(theta), y, radius * std::sin(theta)), XMFLOAT3(std::cos(theta), 0.0f, std::sin(theta));
                    geoData.normals[vIndex] = XMFLOAT3(std::cos(theta), 0.0f, std::sin(theta));
                    geoData.tangents[vIndex] = XMFLOAT4(-std::sin(theta), 0.0f, std::cos(theta), 1.0f);
        ...
        //
        {
            ...
            auto offset = static_cast<uint32_t>(vIndex);
            ...
            {
                theta = float(i) * per_theta;
                float u = std::cos(theta) * radius / height + 0.5f;
                float v = std::sin(theta) * radius / height + 0.5f;
                geoData.vertices[vIndex] = XMFLOAT3(radius * std::cos(theta), h2, radius * std::sin(theta));
                geoData.normals[vIndex] = XMFLOAT3(0.0f, 1.0f, 0.0f);
                geoData.tangents[vIndex] = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
    ...
    GeometryData create_cone(uint32_t entity_id, float radius, float height, uint32_t slices)
    {
        ...
        //
        {
            ...
            for (uint32_t i = 0; i < slices; ++i)
            {
                theta = float(i) * per_theta + per_theta / 2.0f;
                geoData.vertices[vIndex] = XMFLOAT3(0.0f, h2, 0.0f);
                geoData.normals[vIndex] = XMFLOAT3(radius * std::cos(theta) / len, height / len, radius * std::sin(theta) / len);
                geoData.tangents[vIndex] = XMFLOAT4(-std::sin(theta), 0.0f, std::cos(theta), 1.0f);
                geoData.texcoords[vIndex++] = XMFLOAT2(0.5f, 0.5f);
            }
            ...
            for (uint32_t i = 0; i < slices; ++i)
            {
                ...
                geoData.normals[vIndex] = XMFLOAT3(radius * std::cos(theta) / len, height / len, radius * std::sin(theta) / len);
                geoData.tangents[vIndex] = XMFLOAT4(-std::sin(theta), 0.0f, std::cos(theta), 1.0f);
                geoData.texcoords[vIndex++] = XMFLOAT2(std::cos(theta) / 2 + 0.5f, std::sin(theta) / 2 + 0.5f);
            }
                            ...
                            const std::function<DirectX::XMFLOAT4(float, float)>& colorFunc)
    {
        ...
        float sliceWidth = gridSize.x / float(slices.x);
        float sliceDepth = gridSize.y / float(slices.y);
        float leftBottomX = -gridSize.x / 2.0f;
        float leftBottomZ = -gridSize.y / 2.0f;
        float posX, posZ;
        float sliceTexWidth = maxTexCoord.x / float(slices.x);
        float sliceTexDepth = maxTexCoord.y / float(slices.y);
        ...
        {
            posZ = leftBottomZ + float(z) * sliceDepth;
            for (uint32_t x = 0; x <= slices.x; ++x)
            {
                posX = leftBottomX + float(x) * sliceWidth;
                // 计算法向量并归一化
                ...
                XMStoreFloat4(&tangent, XMVector3Normalize(XMVectorSet(normal.y, -normal.x, 0.0f, 1.0f)));
                ...
                geoData.texcoords[vIndex++] = XMFLOAT2(float(x) * sliceTexWidth, maxTexCoord.y - float(z) * sliceTexDepth);
            }
        }
        // 放入索引
